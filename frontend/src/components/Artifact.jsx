...
function Artifact() {
  ...
  const safeIndex = (activeFile >= 0 && activeFile < files.length) ? activeFile : 0
    ...
    <div className='flex flex-col h-full bg-[#0d0f14]'>
      <div className='h-14 px-4 border-b border-white/[0.06] flex items-center gap-3 shrink-0'>
        ...
        <div className='flex items-center gap-2 flex-1 min-w-0'>
          ...
          <div className='text-[13px] font-medium text-slate-200 truncate'>{currentArtifact?.title || "Generated Code"}</div>
        </div>
        ...
        <div className='flex h-auto border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0'>
          {files.map((f, index) => (
            <button
              ...
              className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r border-white/[0.05] relative cursor-pointer bg-transparent ${safeIndex === index ? "text-indigo-400" : "text-slate-500 hover:text-slate-300"}`}
            >
              {f?.name}
              {safeIndex === index && <div className='absolute bottom-0 left-0 right-0 h-[2px] bg-indigo-500 rounded-t-full' />}
            </button>
          ))}
        </div>
      )}
    ...
    <>
      ...
      >
        {!collapsed ? (
          ...
          <div className='flex h-full border-l border-white/[0.06] bg-[#0d0f14] flex-col items-center py-4 gap-3 shrink-0 w-12'>
            <button
              className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0'
